...
export const removeAdmin = createAsyncThunk('auth/removeAdmin', async (userId: number) => {
...
export const fetchTransactions = createAsyncThunk(
  ...
  async (userId: number) => {
    ...
    const validTransactions = (data.transactions || []).filter(
      (tx: Transaction) => tx && tx.id && typeof tx.amount === 'number' && tx.date && tx.status && tx.recipient
    );
...
export const initiateTransaction = createAsyncThunk(
  ...
  async (transactionData: { userId: number; amount: number; recipient: string }) => {
